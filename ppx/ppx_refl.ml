...
module ReflValue (Value : Metapp_utils.ValueS) = struct
  ...
  let cnil = refl_dot "CNil"

  let ccons = refl_dot "CCons"

  let choices_of_list l =
    typed_vector_of_list (construct cnil []) ccons l
...
module ReflValueVal = ReflValue (Metapp_utils.Val)
...
let structure_of_constr structure_of_type context ?rec_type
  ...
  let t, desc =
    ...
    | Some (index, { desc_name; recursive; _ }) ->
        ...
        context.rec_type_refs |> apply_ref (IntSet.add index);
  ...
  if type_args_regular context args then
    ...
    if rec_type = None then
      context.constraints |> apply_ref (fun constraints ->
...
let accessors_of_row_field (ty : Parsetree.core_type Lazy.t) i
  ...
  let module Values (Value : Metapp_utils.ValueS) = struct
    ...
    let sequence, variant =
      match row_field with
      | { prf_desc = Rtag (label, _, []); _ } ->
          sequence_of_list [], variant label.txt None
      | { prf_desc = Rtag (label, _, _); _ } ->
...
let structure_of_variant structure_of_type context
  ...
  let ty = lazy begin
    let fields = fields |> List.map begin fun (field : Parsetree.row_field) ->
      match field with
      | { prf_desc = Rtag (label, _, list); _ } ->
          let list =
            match list with
            | [] -> []
            | _ :: _ -> [[%type: _]] in
          { field with prf_desc = Rtag (label, false, list)}
      | { prf_desc = Rinherit t; _ } -> { field with prf_desc = Rinherit t }
    end in
...
let structure_of_builtins_or_constr structure_of_type context
  ...
  | [%type: [%t? subtype] array] | [%type: [%t? subtype] Array.t] ->
      context.constraints |> apply_ref (Constraints.add_direct_kind "Array");
...
let structure_of_object_field structure_of_type context
    (object_field : Parsetree.object_field)
    : (Parsetree.core_type * Parsetree.expression) *
    ((Parsetree.pattern * Parsetree.class_field) * Parsetree.expression) =
  Ast_helper.with_default_loc object_field.pof_loc @@ fun () ->
  match object_field with
  | { pof_desc = Otag (label, argument); _ } ->
  ...
  | { pof_desc = Oinherit _; _ } ->
      Location.raise_errorf ~loc:object_field.pof_loc
        "ppx_refl does not support object inheritance"

let delays_dot = refl_dot "Delays"

let structure_of_object structure_of_type context
    (fields : Parsetree.object_field list)
    : Parsetree.core_type * Parsetree.expression =
  let methods =
...
let instantiate_with_free accu map _loc name =
  ...
  | Some name ->
      ...
      | None ->
          accu |> apply_ref (StringSet.add name);
...
let make_attributes context ty attributes : Parsetree.expression =
  let cases =
    attributes |> List.map begin fun (attribute : Parsetree.attribute) ->
      let name = lid_of_attr_name attribute.attr_name.txt in
      let expr =
...
let rec structure_of_type context (ty : Parsetree.core_type)
  ...
  let transform () =
    ...
    | [%type: _] ->
        let var = free_variable context in
        context.constraints |> apply_ref begin
    ...
    | { ptyp_desc = Ptyp_var s; _ } ->
        begin match StringIndexer.find_opt s context.vars with
        | Some i ->
            context.constraints |> apply_ref begin fun c -> c |>
    ...
    | { ptyp_desc = Ptyp_constr (constr, args); _ } ->
        begin match
          ...
          Metapp_utils.find_attr "nobuiltin" ty.ptyp_attributes
        with
        | None, None ->
    ...
    | { ptyp_desc = Ptyp_poly (vars, ty); _ } ->
        let context = { context with vars = context.vars |>
          StringIndexer.union (StringIndexer.of_list
            (vars |>
              List.map (fun (var : _ Location.loc) -> Some var.txt)))} in
  ...
  match Metapp_utils.chop_attr "opaque" ty.ptyp_attributes with
  | None -> transform_attr ty.ptyp_attributes
  | Some (_, attributes) ->
      context.constraints |> apply_ref (Constraints.add_direct_kind "Opaque");
...
"Type constructor '%s' has %d parameters but %d arguments given"
      ...
      if eqs <> [] then
        context.constraints |> apply_ref (Constraints.add_direct_kind "GADT");
...
let structure_of_label_declaration context prefix single_label
  ...
  | { ptyp_desc = Ptyp_poly (vars, field_type); _ } ->
      context.constraints |> apply_ref (Constraints.add_direct_kind "Poly");
      let free_variables =
        StringIndexer.of_list
...
let make_constructor_kind context
  ...
  let items = List.mapi (fun i _ -> Metapp_utils.Val.var (item i)) args in
  match constructor.pcd_args with
  | Pcstr_tuple _ ->
...
let structure_of_exists single_constructor i context
    (constructor : Parsetree.constructor_declaration)
  ...
  let constructor_with_args =
    Metapp_utils.Val.force_construct
  ...
  let composed, value_type_name, type_declarations =
    if single_constructor then
      ...
      Metapp_utils.Val.construct (Lident branch_constructor) items,
      branch_name,
  ...
  context.constraints |> apply_ref (Constraints.add_direct_kind "Exists");
  let choice = ReflValueVal.choice_of_int i composed in
...
let structure_of_constructor single_constructor context i
    (constructor : Parsetree.constructor_declaration)
    : (((Parsetree.core_type * Parsetree.expression)
        * Parsetree.core_type) *
         (Parsetree.case * Parsetree.case))
  ...
  try
    ...
    let value_eqs =
      List.init eq_count
        (fun _ -> Metapp_utils.Val.construct (refl_dot "Eq") []) in
    let sequence =
      Metapp_utils.Val.tuple
        [ReflValueVal.sequence_of_list destructs;
          ReflValueVal.sequence_of_list value_eqs] in
    let choice = ReflValueVal.choice_of_int i sequence in
  ...
  with (Exists (loc, name)) ->
    match constructor.pcd_res with
    | Some ty ->
        structure_of_exists single_constructor i context constructor ty
    | None ->
        match name with
        | None ->
...
let structure_of_constr context
  ...
  let cases =
    List.mapi (structure_of_constructor single_constructor context)
      constructors in
  ...
  let construct, destruct = List.split accessors in
  let construct = construct @ irrefutable () in
  let choice_ty = [%type: [%t type_sequence_of_list choices] Refl.choice] in
  ...
  context.constraints |> apply_ref (Constraints.add_direct_kind "Constr");
  let expr =
    [%expr Refl.Constr {
      constructors = [%e ReflValueExp.choices_of_list descs];
  ...
  ([%type: [`Constr of [%t type_sequence_of_list types]]], expr),
...
let structure_of_record context (labels : Parsetree.label_declaration list)
  ...
  context.constraints |> apply_ref (Constraints.add_direct_kind "Record");
  let single_label = is_singleton labels in
  let structures =
  ...
  let expr mapper e =
    ...
    | [%expr Refl.SubGADT ([%e? desc] : [%t? base] -> [%t? sub])] ->
        if !(context.eqs_counter) = 0 then
          ...
          let constructor =
            Metapp_utils.Val.construct (Lident constructor_name) [] in
          type_extensions := (
...
(*
  ...
  let desc_sig =
    Ast_helper.Sig.value (Ast_helper.Val.mk { loc; txt = rec_arity_name }
...
let get_derivers (attributes : Parsetree.attributes)
  ...
  match Metapp_utils.find_attr "deriving" attributes with
  | None -> []
  | Some derivers ->
      let derivers =
        Metapp_utils.expression_of_payload
          (Metapp_utils.attr_payload derivers) in
